(function () {
  ...
  function renderDataOverview() { const counts = state.manifest.source_counts; const quality = state.quality; if ($("data-overview")) { $("data-overview").innerHTML = `<div class="kpi"><strong>${quality.models.length}</strong><span>models</span></div><div class="kpi"><strong>${counts.images}</strong><span>images</span></div><div class="kpi"><strong>${counts.datasets}</strong><span>datasets</span></div>`; } }
  function renderQuality() { const q = state.quality; const status = q.status === "pass" ? "Passed" : "Review required"; if ($("quality-status")) { $("quality-status").innerHTML = `<span class="status-badge ${q.status}">${status}</span>`; } }
  function renderSourceManifest() { /* retained for backward compatibility but not called by loadData anymore */ if (!$("source-body")) return; $("source-body").innerHTML = state.manifest.sources.map((source) => `<tr><td>${escapeHtml(source.file.replace("source/", ""))}</td><td><span class="role-pill ${source.role}">${escapeHtml(source.role)}</span></td><td class="numeric">${source.rows}</td><td>${escapeHtml(source.usage || "-")}</td></tr>`).join(""); }
  ...
  function renderFormula(options) { const w = RankingScoring.normalizeWeights(options.weights); if ($("formula-text")) { $("formula-text").innerHTML = `<div><strong>1. Pixel score:</strong> F<sub>β</sub> = (1+β²)·Precision·Sensitivity / (β²·Precision + Sensitivity) ...</div>`; } }
  ...
  function renderModelCards() { const c = $("model-cards"); if (!c) return; c.innerHTML = ""; state.scored.rows.forEach((row, index) => { const card = document.createElement("article"); card.className = `model-card ${index === 0 ? "winner" : ""}`; const err = errorProfileFor(row.model); const errs = dominantErrors(err).map(([k, v]) => `<li>${escapeHtml(k)}: ${pct(v)}</li>`).join(""); card.innerHTML = `<h3>${escapeHtml(row.model)}</h3><p>Final score: <strong>${fmt(row.finalScore,2)}</strong></p><ul>${errs}</ul>`; c.appendChild(card); }); }
  ...
  function bindEvents() {
    ...
    const rerun = $("rerun-stability"); if (rerun) rerun.addEventListener("click", () => { /* rerun removed */ });
  }
